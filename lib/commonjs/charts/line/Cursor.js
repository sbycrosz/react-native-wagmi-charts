...
function LineChartCursor({
  ...
  const xValues = React.useMemo(() => data.map(({
    timestamp
  }, i) => xDomain ? timestamp : i), [data, xDomain]); // Same scale as in /src/charts/line/utils/getPath.ts

  const scaleX = React.useMemo(() => {
    const domainArray = xDomain !== null && xDomain !== void 0 ? xDomain : [0, xValues.length];
  ...
  const linearScalePositionAndIndex = ({
    ...
    const newXPosition = (closestIndex > 0 ? parsedPath.curves[closestPathCurve].to : parsedPath.move).x; // Update values
  ...
  const onGestureEvent = (0, _reactNativeReanimated.useAnimatedGestureHandler)({
    onActive: ({
      ...
      if (parsedPath) {
        ...
        isActive.value = true; // on Web, we could drag the cursor to be negative, breaking it
